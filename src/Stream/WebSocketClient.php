...
class WebSocketClient extends Client // implements SocketClientImplementation
{	
	/**
     * Constructor takes host, port, path and origin as arguments.
     * 
     * @param string $host
     * @param string $port
     * @param string $path
	...
	{				
		...
		// 			
		$this->resource = fsockopen($this->host, $this->port, $errno, $errstr, 2);
		socket_set_timeout($this->resource, $timeout);
		@fwrite($this->resource, $header);
		$response = @fread($this->resource, 1500);
